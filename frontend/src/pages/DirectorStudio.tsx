...
export default function DirectorStudio() {
  ...
  return <main className="min-h-[100dvh] workbench-shell p-4 md:p-6"><div className="max-w-7xl mx-auto space-y-4">
    <header className="card-cinematic p-5 flex flex-wrap items-center gap-4"><button className="icon-button" title="返回智能助手" onClick={() => navigate('/director')}><ArrowLeft size={18}/></button><div className="home-brand-mark"><Robot size={20} weight="fill"/></div><div className="flex-1 min-w-[240px]"><p className="section-index">PRIVATE DIRECTOR STUDIO</p><h1 className="text-2xl font-heading">我的导演方法</h1><p className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>把 GPT 讨论成果变成可试跑的 Skill，再组装成你自己的导演包。</p></div><button className="btn-gold text-xs" onClick={() => { setTab('import'); setGptText(EXAMPLE_GPT_DISCUSSION) }}><Plus size={15}/> 导入 GPT 讨论成果</button></header>

    <div className="grid md:grid-cols-3 gap-2"><button className={tab === 'import' ? 'btn-gold justify-center' : 'btn-cinematic justify-center'} onClick={() => setTab('import')}><FileText size={16}/> 导入 GPT 讨论成果</button><button className={tab === 'skills' ? 'btn-gold justify-center' : 'btn-cinematic justify-center'} onClick={() => setTab('skills')}><Robot size={16}/> 我的 Skill ({skills.length})</button><button className={tab === 'packs' ? 'btn-gold justify-center' : 'btn-cinematic justify-center'} onClick={() => setTab('packs')}><Package size={16}/> 我的导演包 ({packs.length})</button></div>
    ...
    {tab === 'skills' && <section className="grid xl:grid-cols-[300px_minmax(0,1fr)_340px] gap-4"><aside className="card-cinematic p-4 space-y-2"><div className="flex items-center justify-between"><p className="label-cinematic">私有 Skill</p><button className="icon-button" title="导入 GPT 讨论成果" onClick={() => setTab('import')}><Plus size={16}/></button></div>{skills.map(skill => <button key={skill.skillId} onClick={() => chooseSkill(skill)} className="w-full text-left p-3 border rounded transition-colors" style={{ borderColor: selectedSkill?.skillId === skill.skillId ? 'var(--accent)' : 'var(--border)', background: selectedSkill?.skillId === skill.skillId ? 'var(--bg-elevated)' : 'var(--bg-surface)' }}><div className="flex justify-between gap-2"><b className="truncate">{skill.name}</b><span className="status-pill text-[10px]" data-tone={skill.status === 'published' ? 'ok' : skill.status === 'disabled' ? 'danger' : 'warn'}>{skill.status}</span></div><p className="text-[11px] mt-1 line-clamp-2" style={{ color: 'var(--text-muted)' }}>v{skill.version} · {skill.description}</p></button>)}{!skills.length && <p className="text-xs p-3" style={{ color: 'var(--text-muted)' }}>尚无 Skill。先导入一段 GPT 讨论成果。</p>}</aside>
