...
export const api = {
  ...
  listPresentationTemplates: () => request<{ templates: any[] }>('/structured/presentation-templates'),
  proposeVisualPlan: (projectId: string, settings: any = {}) => request<any>(`/projects/${projectId}/visual-plan/propose`, { method: 'POST', body: JSON.stringify({ mode: 'target_duration', targetDuration: 7, ...settings }) }),
  ...
  listDirectorStudioSkills: () => request<any>('/director-studio/skills'),
  getDirectorStudioSkill: (skillId: string, version?: number) => request<any>(`/director-studio/skills/${encodeURIComponent(skillId)}${version ? `?version=${version}` : ''}`),
  importGptSkill: (text: string, name?: string) => request<any>('/director-studio/skills/import-gpt', { method: 'POST', body: JSON.stringify({ text, name }) }),
  updateDirectorStudioSkill: (skillId: string, data: any) => request<any>(`/director-studio/skills/${encodeURIComponent(skillId)}`, { method: 'PUT', body: JSON.stringify(data) }),
  validateDirectorStudioSkill: (skillId: string, version?: number) => request<any>(`/director-studio/skills/${encodeURIComponent(skillId)}/validate`, { method: 'POST', body: JSON.stringify({ version }) }),
  trialDirectorStudioSkill: (skillId: string, version?: number) => request<any>(`/director-studio/skills/${encodeURIComponent(skillId)}/trial`, { method: 'POST', body: JSON.stringify({ version }) }),
  publishDirectorStudioSkill: (skillId: string) => request<any>(`/director-studio/skills/${encodeURIComponent(skillId)}/publish`, { method: 'POST' }),
