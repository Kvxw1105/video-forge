...
def _new_manifest(batch_id: str, spec: TemplateBatchSpec, spec_hash: str) -> dict:
    now = _now()
    director_pack = spec.directorPack.model_dump() if spec.directorPack else None
    return {"schemaVersion": 1, "batchId": batch_id, "name": spec.name, "idempotencyKey": spec.idempotencyKey, "specHash": spec_hash, "productionMode": spec.productionMode, "productionProfile": spec.productionProfile, "directorPack": director_pack, "status": "queued", "progress": 0, "totalItems": len(spec.items), "succeededItems": 0, "failedItems": 0, "skippedItems": 0, "currentItemId": None, "createdAt": now, "updatedAt": now, "items": [{"itemId": item.itemId, "status": "queued", "phase": "queued", "projectId": None, "previewUrl": None, "jianyingDraftPath": None, "jianyingZipPath": None, "duration": None, "visualPlanId": None, "generationPackPath": None, "visualBatchId": None, "visualBatchIds": [], "productionProfile": spec.productionProfile, "directorPack": director_pack, "resolvedDirectorPolicy": None, "productionMode": spec.productionMode, "expectedScenes": [], "visualCoverage": None, "outputs": {"preview": None, "jianying": None}, "attempts": 0, "errorCode": None, "error": None, "startedAt": None, "finishedAt": None} for item in spec.items]}
