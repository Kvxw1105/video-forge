...
def main() -> int:
    ...
    assert run["approvals"][0]["recipeStepId"] == "bind_assets"
    ...
    assert final["labActions"][0]["tool"] == "bind_scene_assets", final
    ...
    assert any(item["type"] == "lab.action.succeeded" for item in replay)
